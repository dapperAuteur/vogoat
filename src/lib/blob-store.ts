...
const vercelStore: TakeAudioStore = {
  ...
  async get(url) {
    ...
    const result = (await get(url, { token: env.BLOB_READ_WRITE_TOKEN } as Parameters<typeof get>[1])) as unknown;
    ...
    const res = await fetch(target, { headers: { authorization: `Bearer ${env.BLOB_READ_WRITE_TOKEN ?? ""}` } });
