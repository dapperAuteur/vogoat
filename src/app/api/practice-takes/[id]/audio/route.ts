...
export async function GET(request: NextRequest, ctx: RouteContext<"/api/practice-takes/[id]/audio">) {
  ...
  if (wantsDownload && !hasPaidPerks(session.user as SessionUser)) {
    return NextResponse.json({ ok: false, error: "Practice downloads come with paid plans.", code: "paid_only" }, { status: 403 });
  }
  let bytes: Uint8Array | null = null;
  try {
    bytes = await getTakeAudioStore().get(row.blobUrl);
  } catch (error: unknown) {
    await logAppError(db, { source: "server", message: `store.get: ${error instanceof Error ? error.message : "unknown"}`, path: "/api/practice-takes/[id]/audio" });
